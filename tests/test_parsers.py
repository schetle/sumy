"""Tests for plaintext and HTML parsers."""
...
def test_parse_plaintext():
    """Test parsing a plaintext string into paragraphs and sentences."""
...
def test_parse_plaintext_long():
    """Test parsing a longer plaintext string with multiple paragraphs."""
    parser = PlaintextParser.from_string("""
        Ako sa máš? Ja dobre! A ty? No
        mohlo to byť aj lepšie!!! Ale pohodička.
...
def test_annotated_text():
    """Test parsing an HTML file with headings and paragraphs.

    With readability-lxml, headings are extracted into their own paragraph
    rather than being merged with the following content paragraph.
    """
    ...
    assert str(document.paragraphs[2].sentences[0]) == \
        "Tento text je tu aby vyplnil prázdne miesto v srdci súboru."
    assert str(document.paragraphs[2].sentences[1]) == \
