...
def test_args_none():
    with pytest.raises(DocoptExit):
        docopt(str(main_doc), None, version=__version__)
...
def test_args_two_commands():
    with pytest.raises(DocoptExit):
        docopt(str(main_doc), 'lsa luhn'.split(), version=__version__)


def test_args_url_and_file():
    with pytest.raises(DocoptExit):
        docopt(str(main_doc), 'lsa --url=URL --file=FILE'.split(), version=__version__)
