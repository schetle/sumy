"""Tests for the SumBasic summarizer."""
...
def _build_summarizer(stop_words):
    """Build a SumBasicSummarizer with given stop words.

    Args:
        stop_words: List of stop words to filter.
...
def test_empty_document():
    """Test SumBasic with an empty document."""
...
def test_single_sentence():
    """Test SumBasic with a single sentence."""
...
def test_filter_out_stop_words():
    """Test that stop words are filtered from a word list."""
...
def test_get_all_content_words_in_doc():
    """Test extraction of all content words from document sentences."""
    summarizer = _build_summarizer(EMPTY_STOP_WORDS)
...
def test_compute_tf():
    """Test term frequency computation over document sentences."""
    summarizer = _build_summarizer(EMPTY_STOP_WORDS)
...
def test_compute_average_probability_of_words():
    """Test computation of average word probability for a sentence."""
    summarizer = _build_summarizer(EMPTY_STOP_WORDS)
    ...
    assert summarizer._compute_average_probability_of_words(word_freq, s0) == \
        pytest.approx(0, abs=0.0001)
    assert summarizer._compute_average_probability_of_words(word_freq, s1) == \
        pytest.approx(1 / 6, abs=0.0001)
    assert summarizer._compute_average_probability_of_words(word_freq, s2) == \
        pytest.approx(5 / 12, abs=0.0001)
    assert summarizer._compute_average_probability_of_words(word_freq, s3) == \
        pytest.approx(8 / 18, abs=0.0001)


def test_compute_ratings():
    """Test that SumBasic computes sentence ratings correctly."""
    summarizer = _build_summarizer(EMPTY_STOP_WORDS)
