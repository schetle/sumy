...
def test_compute_tf():
    ...
    assert freq["kicking"] == pytest.approx(1/6)
    assert freq["soccer"] == pytest.approx(1/6)
    assert freq["balls"] == pytest.approx(1/6)
    assert freq["eating"] == pytest.approx(1/6)
    assert freq["chicken"] == pytest.approx(1/6)
    assert freq["dumplings"] == pytest.approx(1/6)

    document = build_document([s0, s0, s1])
    freq = summarizer._compute_tf(document.sentences)
    assert freq["kicking"] == pytest.approx(2/9)
    assert freq["soccer"] == pytest.approx(2/9)
    assert freq["balls"] == pytest.approx(2/9)
    assert freq["eating"] == pytest.approx(1/9)
    assert freq["chicken"] == pytest.approx(1/9)
    assert freq["dumplings"] == pytest.approx(1/9)
...
def test_compute_average_probability_of_words():
    ...
    assert summarizer._compute_average_probability_of_words(word_freq, s0) == pytest.approx(0)
    assert summarizer._compute_average_probability_of_words(word_freq, s1) == pytest.approx(1/6)
    assert summarizer._compute_average_probability_of_words(word_freq, s2) == pytest.approx(5/12)
    assert summarizer._compute_average_probability_of_words(word_freq, s3) == pytest.approx(8/18)
