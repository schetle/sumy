...
def test_two_sentences():
    ...
    assert str(returned[0]) == "I am that 1. sentence"
...
def test_stop_words_correctly_removed():
    ...
    expected = []
    assert summarizer._to_words_set(sentences[0]) == expected
    assert summarizer._to_words_set(sentences[1]) == expected
    assert summarizer._to_words_set(sentences[2]) == expected
    assert summarizer._to_words_set(sentences[3]) == expected

    assert summarizer._to_words_set(sentences[4]) == ["some", "relevant", "sentence"]
    assert summarizer._to_words_set(sentences[5]) == ["some", "more", "relevant", "sentence"]
