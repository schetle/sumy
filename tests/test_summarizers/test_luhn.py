...
class TestLuhn(unittest.TestCase):
    ...
    def test_real_example(self):
        ...
        self.assertEqual(len(returned), 2)
        self.assertEqual(str(returned[0]), "Jednalo se o případ chlapce v 6. třídě, který měl problémy s učením.")
        self.assertEqual(
            str(returned[1]), "Připadal si, že je mezi malými dětmi a realizoval se tím, že si ve třídě o rok mladších dětí budoval vedoucí pozici."
        )
...
class TestSentenceRating(unittest.TestCase):
    def setUp(self):
        ...
        self.sentence = build_sentence("Nějaký muž šel kolem naší zahrady a žil pěkný život samotáře")
