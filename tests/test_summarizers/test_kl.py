...
def test_joint_freq():
    ...
    assert freq["one"] == pytest.approx(1.0/4)
    assert freq["two"] == pytest.approx(1.0/4)
    assert freq["three"] == pytest.approx(1.0/4)
    assert freq["four"] == pytest.approx(1.0/4)

    w1 = ["one", "two", "three", "four"]
    w2 = ["one", "one", "three", "five"]
    freq = summarizer._joint_freq(w1, w2)
    assert freq["one"] == pytest.approx(3.0/8)
    assert freq["two"] == pytest.approx(1.0/8)
    assert freq["three"] == pytest.approx(1.0/4)
    assert freq["four"] == pytest.approx(1.0/8)
    assert freq["five"] == pytest.approx(1.0/8)
...
def test_kl_divergence():
    ...
    assert abs(summarizer._kl_divergence(w1, w2) - kl_correct) < 0.00001
