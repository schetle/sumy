...
def test_cosine_exact_match():
    text = "Toto je moja veta, to sa nedá poprieť."
    model = TfDocumentModel(text, Tokenizer("czech"))
    assert abs(cosine_similarity(model, model) - 1.0) < 1e-7
...
def test_cosine_no_match():
    ...
    assert abs(cosine_similarity(model1, model2) - 0.0) < 1e-7
...
def test_cosine_half_match():
    ...
    assert abs(cosine_similarity(model1, model2) - 0.5) < 1e-7
...
def test_unit_overlap_exact_match():
    ...
    assert abs(unit_overlap(model, model) - 1.0) < 1e-7
...
def test_unit_overlap_no_match():
    ...
    model1 = TfDocumentModel("Toto je moja veta. To sa nedá poprieť!", tokenizer)
    model2 = TfDocumentModel("Hento bolo jeho slovo, ale možno klame.", tokenizer)
    assert abs(unit_overlap(model1, model2) - 0.0) < 1e-7
...
def test_unit_overlap_half_match():
    ...
    model1 = TfDocumentModel("Veta aká sa tu len veľmi ťažko hľadá", tokenizer)
    model2 = TfDocumentModel("Teta ktorá sa tu iba veľmi zle hľadá", tokenizer)
    assert abs(unit_overlap(model1, model2) - 1/3) < 1e-7
...
def test_rouge_n():
    ...
    assert abs(rouge_n(candidate, reference1, 1) - 4/10) < 1e-7
    assert abs(rouge_n(candidate, reference2, 1) - 1/10) < 1e-7

    assert abs(rouge_n(candidate, reference1, 2) - 3/9) < 1e-7
    assert abs(rouge_n(candidate, reference2, 2) - 0/9) < 1e-7

    assert abs(rouge_n(candidate, reference1, 3) - 2/8) < 1e-7
    assert abs(rouge_n(candidate, reference2, 3) - 0/8) < 1e-7

    assert abs(rouge_n(candidate, reference1, 4) - 1/7) < 1e-7
    assert abs(rouge_n(candidate, reference2, 4) - 0/7) < 1e-7
...
def test_union_lcs():
    ...
    assert abs(_union_lcs(candidates, reference[0]) - 4/5) < 1e-7
