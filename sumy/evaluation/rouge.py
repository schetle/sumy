...
def _get_index_of_lcs(x: Sequence[str], y: Sequence[str]) -> tuple[int, int]:
	return len(x), len(y)


def _len_lcs(x: Sequence[str], y: Sequence[str]) -> int:
	'''
...
def _lcs(x: Sequence[str], y: Sequence[str]) -> dict[tuple[int, int], int]:
	'''
...
def _recon_lcs(x: Sequence[str], y: Sequence[str]) -> tuple[str, ...]:
	'''
	...
	table = _lcs(x, y)
	def _recon (i: int, j: int) -> list[str]:
		if i == 0 or j == 0:
			return []
		elif x[i-1] == y[j-1]:
...
def rouge_n(evaluated_sentences: Sequence[Sentence], reference_sentences: Sequence[Sentence], n: int = 2) -> float:
	"""
...
def _f_lcs(llcs: int, m: int, n: int) -> float:
	'''
...
def rouge_l_sentence_level(evaluated_sentences: Sequence[Sentence], reference_sentences: Sequence[Sentence]) -> float:
	"""
...
def rouge_l_summary_level(evaluated_sentences: Sequence[Sentence], reference_sentences: Sequence[Sentence]) -> float:
	"""
