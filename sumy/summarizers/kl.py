...
class KLSummarizer(AbstractSummarizer):
    ...
    def _get_ratings(self, document: ObjectDocumentModel) -> dict[Sentence, int]:
        sentences = document.sentences

        ratings = self._compute_ratings(sentences)
        return ratings

    def _get_all_words_in_doc(self, sentences: tuple[Sentence, ...] | list[Sentence]) -> list[str]:
    ...
    def _joint_freq(self, word_list_1: list[str], word_list_2: list[str]) -> dict[str, float]:
        ...
        joint: dict[str, float] = {k: float(v) for k, v in wc1.items()}
    ...
    def _kl_divergence(self, summary_freq: dict[str, float], doc_freq: dict[str, float]) -> float:
        '''
    ...
    def _find_index_of_best_sentence(self, kls: list[float]) -> int:
        '''
